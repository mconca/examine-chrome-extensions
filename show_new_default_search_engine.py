...
if __name__=='__main__':
    ...
    if source == 'chrome' or source == 'firefox':
        k = 0
        print('Name,Users,ID,search name,is_default,search URL')
        print('---------------------')
        for filename in os.listdir('extensions/'+source+'-manifests'):
            ...
            if ext.has_cuo:
                ...
                if 'search_provider' in cso:
                    sp = cso['search_provider']
                    if 'is_default' in sp:
                        isd = sp['is_default']
                    else:
                        isd = False
                ...
                if not res['Name']:
                    res['Name'] = filename
                try:
                    print(res['Name'], ',', res['Users'], ',', res['ID'], ',', search_name + ',', isd, ',', surl)
                except:
                    print(filename, ',', res['Users'], ',', res['ID'], ',', search_name + ',', isd, ',', surl)
